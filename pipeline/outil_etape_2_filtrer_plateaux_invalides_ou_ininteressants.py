...
class FiltrerLesPlateaux:
    ...
    def filtrer_les_plateaux(self, nb_colonnes, nb_lignes):
        ...
        lot_de_plateaux_analyse = LotDePlateaux((nb_colonnes, nb_lignes, self._nb_colonnes_vides),
                                        repertoire_export_json=self._repertoire_analyse,
                                        nb_plateaux_max = self._memoire_max)
        if lot_de_plateaux_analyse.est_deja_termine:
            # Copie des fichiers
            if self._repertoire_analyse != self._repertoire_filtre:
                self.copier_les_plateaux(lot_de_plateaux_analyse.chemin_enregistrement)
            lot_de_plateaux_analyse = None
            ...
            self._done = False
            logger.info(f"La recherche des plateaux n'est pas achevee.")
